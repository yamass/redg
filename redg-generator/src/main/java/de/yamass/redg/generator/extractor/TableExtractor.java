...
public class TableExtractor {
	...
	public static final String DEFAULT_TARGET_PACKAGE = "de.yamass.redg.generated";
	...
	public TableModel extractTableModel(DataTypeLookup dataTypeLookup, final Table table) {
		...
		model.setForeignKeys(table.getImportedForeignKeys().stream() // only get data about "outgoing" foreign keys
				.filter(foreignKey -> {
					Set<String> foreignKeyColumnNames = foreignKey.getColumnReferences().stream()
				...
				// Sort them by order of declaration!
				// TODO: make this configurable (alphabetical, declaration order, custom)
				.sorted(Comparator.comparing(foreignKey -> table.getColumns().indexOf(foreignKey.getColumnReferences().get(0).getForeignKeyColumn())))
